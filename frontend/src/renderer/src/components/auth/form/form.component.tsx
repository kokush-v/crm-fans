...
import { QUERY_KEYS, ROUTER_KEYS } from '../../../constants/app-keys.const'
...
import {
  ...
  showSuccesToast
} from '@renderer/components/form.toasts'
...
export const FormikAuthForm = ({ type }: FormikAuthFormProps) => {
  ...
  const sumbitFunc = {
    login: ({ userName, ...values }: AuthData) =>
      loginMutation(values, {
        ...
        onError: (error) => {
          if (error instanceof AxiosError) {
            showErrorToastWithText(toast, error.response?.data.message)
          } else {
